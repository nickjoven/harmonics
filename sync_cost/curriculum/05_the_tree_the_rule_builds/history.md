...
combining rule. The `edges` demo makes this concrete: each edge of the
tree *carries* one of these generators, so a tree edge is not decoration
between nodes but the group element that produces the child from the
parent. That edge-as-generator is exactly the object Module 6 acts on
when it gives the strip a half-twist.
