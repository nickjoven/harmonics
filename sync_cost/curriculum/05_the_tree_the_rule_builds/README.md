...
So far the tree has been described by its *nodes* — the ratios — and the
address is the list of turns that reaches one. But the turns themselves
carry structure. An **edge** is the step from a ratio to one of its two
children, and a single edge is four things at once. Run
`python3 engine.py --demo edges`:

- **A move.** An `L` edge means "combine with your left boundary,"
  an `R` edge "combine with your right boundary." The child is the
  mediant of the parent with whichever neighbor the turn names — the
  same combining rule, now attached to a direction.
- **A matrix.** Each turn is a fixed 2×2 table of whole numbers,
  ...
  and the turns of an address, multiplied together, give one matrix
  whose **two columns are the ratio's two boundary neighbors**. For
  address `LRRLLLLL` the product is `[[11 2] [16 3]]`; its columns are
  11/16 and 2/3, and the node 13/19 is exactly their mediant. Every one
  of these matrices has determinant 1 — the same `b·c − a·d = 1` neighbor
  invariant from the section above, now seen as a property of the turns
  rather than of the row.
- **A continued-fraction digit.** A run of identical turns — five `L`s in
  a row — is one term of the continued fraction. The edges *are* the
  digits of the address.
- **A bifurcation to a narrower lock.** Give each ratio a *complexity*,
  top + bottom; Module 3 showed the tongue is wider when this is smaller.
  Down **every** edge complexity strictly increases (confirmed here on all
  1022 edges to depth 9), so every turn steps from a lock to a strictly
  *finer* one in the gap beside it. An edge is never lateral.

That is the reading the framework uses: the tree's **nodes are the
mode-locks**, and its **edges are the bifurcations between them** — each
turn a generator that carries you one step deeper into the locking
hierarchy, `L` toward the left neighbor, `R` toward the right. Module 6
takes these two turns and asks what they do to the strip the tree is
drawn on once it is given a half-twist.
