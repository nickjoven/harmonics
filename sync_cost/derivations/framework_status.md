...
| Friedmann form at r = 1 — *canonically affirmed structural*; the original named source was **never committed in any artifact** (exhaustive search: cloud + all local ket stores; original text unrecoverable), but a **forward reconstruction from live canonical dependencies** (`einstein_from_kuramoto` + `adm_dictionary` + `cosmological_cycle`) now provides the home — S1–S2 closed at K = 1, result class **unchanged**, S3–S5 remain the open N9 null; affirmed alongside R, Λ·ℓ_P² | `k_of_t_friedmann.md` (forward reconstruction), `numerology_inventory.md` "What remains genuinely structural" #1 |
