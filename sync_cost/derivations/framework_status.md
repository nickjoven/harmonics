...
- **Bare K=1 substrate-side identities**: substrate-side reference values at K=1 (`duty(q) = 1/q^d`); declined as M_Z predictions. Some carry running-disproof annotations; all 5 sit in `MANIFEST.yml::bare_k1_identities`. (Consolidates the prior "Floor (numerology cloud)" entries with the K=1 entries that had been split into "Fails" — see `Fails` below.)
- **Empirical (open, framework-noted, not derived)**: framework-noted values without derivation; same administrative status as SM free parameters.
- **Fails**: confirmed numerology with explicit disproof. (Now empty: the two prior entries — sin²θ_W = 8/35 and 1/α_em (tree) = 35 — were consolidated into `Bare K=1 substrate-side identities` above with their running-disproof annotations preserved per entry.)
...
| Item | Source |
|---|---|
| a₁(down)²/a₁(lep)² = q₂·q₃ = 6 (down-type double cover; 0.04σ vs PDG 2024). D1 closed via orbit-count theorem (`down_type_phase_d1_eigenvalue.py` verified); D2 structural + numerical, caveat honest in source. Row added 2026-07-25 per #328 Card 1 ruling — the Class 5 label previously had no canonical registry home (the audit's false-cite finding, PR #326). | `down_type_double_cover_closed.md` |
...
| **Preferred-basis dissolution (QM measurement-side foundational consolidation)**: The QM preferred-basis problem dissolves into basin selection — each measurement is the substrate identifying which basin of attraction the system flowed into (`lesson_forced_basin_selection.md`); the basin's natural coordinates *are* the measurement's basis, fixed structurally by the saddle-node normal form `dx/dt = μ − x²` (basin width `Δθ ∝ √ε` exactly, not approximately, because the parabola *is* the normal form at the bifurcation). No global "preferred basis" exists — each measurement context supplies its own via local bifurcation geometry. Completes the framework's measurement-side dissolution program alongside `collapse_dissolution.md` (non-unitary projection) and `figure_eight.md` (Born statistic from `J² = −I`). Class 3 articulation; synthesis of installed substrate apparatus, no new primitive. Explicit row added 2026-07-06 in the substrate-freshness sweep after F2 consolidation — the dissolution was authored but not yet promoted to the ledger. | `preferred_basis_dissolution.md`; `lesson_forced_basin_selection.md`; `collapse_dissolution.md`; `figure_eight.md`; `born_rule.md` |
...
| **D42: Yang-Mills from Klein bottle (gauge-sector Lovelock)**: Yang-Mills equations `D_μ F^μν = J^ν` for gauge group SU(3) × SU(2) × U(1) are the unique gauge dynamics consistent with the Klein bottle's kinematic constraints (Z_6 = Z_2 × Z_3 center, d=3+1, Lorentz invariance, gauge invariance, second-order equations of motion) via Utiyama's theorem + Cartan's classification. Gauge-sector analog of D13 (Einstein from Lovelock). Wiring verified end-to-end (`gauge_lovelock_wiring.py`); five M-gap exposition fixes in the revision. Companion: D21 five-computation program (`discrete_gauge_resolution.md`) confirming Klein-bottle-side premises (anomaly cancellation ✓ via `anomaly_check.py`; Z_6 center ✓ via `fiber_bundle.py`; confinement asymmetry q=2 open / q=3 locked ✓ via `xor_asymmetry.py`) — Yang-Mills DYNAMICS + Klein-bottle KINEMATICS both landed. **F2 audit issue #263 flag was substrate-freshness failure**: D42 and `discrete_gauge_resolution.md` both landed 2026-04-07/2026-04-13, 2+ months before the audit flagged F2 as "conjectural"; the audit-hygiene gap (upstream cites verified, downstream resolutions missed) is worth framework-level attention. F2 epic (#268) closes with consolidation note; PR-1/PR-2/PR-3 added complementary content (K=0 modal-claim confirmation with Kuramoto-pulling tolerance; F-agnostic dimension counting; approximate SU(2)-like near-degeneracy on {B, D} at ~10⁻⁸ orthogonal to `jacobian_v2.py`'s sl(2,R) finding) but no headline closure. | `gauge_sector_lovelock.md` (D42, Derived); `discrete_gauge_resolution.md` (D21 five-computation program); `anomaly_check.py`, `fiber_bundle.py`, `xor_asymmetry.py`, `jacobian_v2.py`; F2 epic #268 with consolidation PR |
| **D13: Einstein equations from Lovelock (gravity-sector uniqueness)**: `G_μν + Λ g_μν = 8πG T_μν` is the unique field equation consistent with the four conditions the Kuramoto→ADM continuum limit at K=1 satisfies (metric theory on 3+1 manifold, self-consistency origin from `N(p/q) = N_total · g · w`, second-order in metric derivatives, general covariance) via Lovelock's theorem (1971). Gravity-sector analog of D42 (Yang-Mills from Klein bottle). Assumption A1 (d = 3 from minimum self-sustaining loop, Derivation 6). Uniqueness had been implicit in `Friedmann form at r = 1` (S3-S5 disposition) and `K=1 ↔ K<1 sector decoupling` rows above/below but was never named as a first-class Survives entry. Explicit row added 2026-07-06 in the substrate-freshness sweep after F2 consolidation — same audit-hygiene pattern as D42/#285: derived, sealed, adjacent-cited, but not promoted to the ledger. | `einstein_from_kuramoto.md` (D13, Theorem established); `PROOF_A_gravity.md` (P8 capstone) |
...
## Bare K=1 substrate-side identities (declined as M_Z predictions)

Substrate-side measure-theoretic consequences of `duty(q) = 1/q^d`
at critical coupling (`duty_dimension_proof.md`), enumerated in
`MANIFEST.yml::bare_k1_identities`. **Reference values at K=1, not
predictions at observational scales** — the 1–3% gaps to M_Z
observables are anchor-side amplification factors per Instance 7
(same shape as A_s, v/M_P, lattice-QCD bare-vs-renormalized). Per
Region C Phase B (`numerology_count_phase_b.md`, verdict
2026-04-26), the cloud is statistically consistent with pigeonhole
at α=0.05; single-instance audits land Class 2 by construction.

| Identity | Value | M_Z residual | Disposition |
|---|---|---|---|
| sin²θ_W = q₂³/(q₂³+q₃³) | 8/35 = 0.22857 | 1.1% vs PDG 0.23121 | Substrate-side bare reference. **Pushed to M_Z and ruled out**: SM running disproof (`sinW_running_check.py`); K-scanning ruled out (`sinw_fixed_point.md`); d_eff = 80/27 ansatz G1-null. Do not use as building block at M_Z. |
| 1/α_em (tree) = q₂³+q₃³ | 35 | factor ~3.7 vs PDG 127.95 | Substrate-side bare reference. **Pushed to M_Z and ruled out**: same SM running analysis as sin²θ_W. Tree-scale identity, not comparable to 1/α_em(M_Z) without anchor-side amplification. Do not use as building block at M_Z. |
| m_H/v = 1/q₂ | 1/2 = 0.5 | 1.7% vs 0.5087 (12.6σ) | Substrate-side bare reference. Not pushed to M_Z; anchor-side amplification declined per Instance 7. |
| λ_H = 1/q₂³ | 1/8 = 0.125 | 3.4% (vs 0.129) | Substrate-side bare reference. Not pushed to M_Z; anchor-side amplification declined per Instance 7. |
| α_s/α_2 = q₃³/q₂³ | 27/8 = 3.375 | 3.2% vs PDG 3.488 | Substrate-side bare reference. Not pushed to M_Z; anchor-side amplification declined per Instance 7. (Distinct from sin²θ_W above: no dedicated disproof script, and none warranted — α_s's large running spoils any tree-scale ratio a priori by *magnitude* (asymptotic freedom), where sin²θ_W's falsifier was a computed *sign*. See `numerology_inventory.md` Class 3.) |

All 5 sit in `MANIFEST.yml::bare_k1_identities` (substrate-side
block, separate from the canonical `scorecard:`). The two with
running disproofs (sin²θ_W, 1/α_em) keep the **"do not use as
building block at M_Z"** warning from the prior `Fails`
disposition; the substrate-side K=1 reference value remains valid
within its declared scope. The 1–3% floor is numerology ensemble,
not structural.
...
| **sin²θ_W (parameter)** | ≈ 0.231 at M_Z | Framework notes; no derivation attempted at the parameter level. The specific identity `sin²θ_W = 8/35` is in `Bare K=1 substrate-side identities` above as a tested-and-failed Planck-scale numerology (SM running disproof preserved per entry), but the underlying parameter remains open. | (none) |
...
(empty after 2026-06 bare-K=1 consolidation; the two prior entries —
sin²θ_W = 8/35 and 1/α_em (tree) = 35 — moved to
`Bare K=1 substrate-side identities` above with their
`sinW_running_check.py` SM-running disproofs preserved as per-entry
annotations. The **"do not use as building block at M_Z"** warning
travels with each.)
...
| Item | Why eliminated |
|---|---|
| v/M_P ≈ 13⁻¹⁵ (3.1%) | `yukawa_mediant_cascade.py` null; `z_30_substrate_check.py` dead end. **Provenance gap (2026-07-21):** both scripts deleted and their cited commits unresolvable in this repo — the nulls are recorded in `depth_connection_nulls.md` (rows 1–2, self-contained) but not reproducible from this tree. |
