...
| **#TICK** | The explicit minimal-tick update rule is not fully specified. Mediant + half-twist + conservation-filters are forced; **phase advance on the active mode** is under-determined in two ways: (a) the K=1 mediant **branch-selector** (Finding 3's *static* S_v is closed, but the *dynamical* selector at K=1 — where tongue-width degenerates — is unpinned); (b) **constructiveness of "continuum-is-shadow"**: a first-order *irreversible* tick does not generically coarse-grain into the *second-order* sine-Gordon shadow — `substrate_determinism.md` asserts momentum-dissolution but never constructs the first→second-order emergence. Leading hypothesis: phase advance is a per-tick **binary Z₂ winding-sign** (kink/antikink; bicone sheet), with Goldstein–Kac telegrapher's → wave-equation as the candidate constructive bridge. | `substrate_determinism.md` (non-constructive momentum-dissolution claim); this session's tick-rule sketch | an explicit context-functional fixing the per-tick Z₂ sign (= the saddle-node branch of `born_rule.md`), and a construction taking the binary tick to the sine-Gordon shadow (telegrapher's-type, with phase-dependent flip supplying the nonlinearity) |
