...
| **#FLOW** | **Competitor #1 posed** (mediant vs. flow). Both readings give Farey structure — non-discriminating. The single decidable difference is the **K=1 complement**: a continuous *critical* flow predicts a parameter-free universal Cantor dimension `D ≈ 0.8700` (sine-circle-map universality) that the discrete mediant generator structurally cannot produce; the (Mediant) reading predicts a trivial/empty complement (transcendentals = shadow). Framework's "Adler-only / not-a-twist-map" eliminations lean *flow-eliminable* but the decisive measurement (K=1 locked-complement Hausdorff dimension) is **not yet run**. Stochastic competitor #2 already eliminated (`#TICK`); this is the *remaining* foundational rival. | `mediant_vs_flow_problem.md`; `framework_status.md` "Eliminated" (Adler-only evidence) | the K=1 locked-complement Hausdorff-dimension measurement: `D≈0.87` ⇒ flow required (discrete-fundamental falsified); trivial/empty ⇒ flow eliminable by parsimony (mediant-fundamental) |
