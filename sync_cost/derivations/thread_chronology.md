...
|---|---|---|---|
| **#INF** | Inflation-era seam **structure**: the geometric `(1−K)√K` form is structurally insufficient ∀K; a non-geometric seam delivering `\|∇K\|_inflation ≈ 2.68` is needed — *or* an honest one-input boundary | `k_inflation_seam_obstruction.md`, `inflation_duration.md` open #2 | a derived non-geometric inflation-era seam form, or explicit acknowledgement that `\|∇K\|_inflation` is an independent input |
