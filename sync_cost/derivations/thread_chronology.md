...
| **#TICK** | **(a) CLOSED, (b) carries all residual.** Phase advance on the active mode = a per-tick **binary Z₂ winding-sign** (kink/antikink; bicone sheet). **(a)** the selector *is* the saddle-node branch — which side of the J-symmetric separatrix the mediant-counted state lies; deterministic, no noise. The stochastic-Kramers competitor is **discriminated** (the `\|ψ\|²` exponent is the topological saddle-node degree — no `kT`; uniform-curvature is forced at the binary by the half-twist `J²=−I` parity; the measure is the mediant counting primitive) — see `born_rule_parameter_free.md`. **(b) OPEN** — the *only* residual: whether the discrete mediant counting measure is *constructively* the source of the apparent continuum measure (first-order irreversible tick → second-order sine-Gordon shadow); leading bridge Goldstein–Kac telegrapher's → wave-equation, nonlinearity from a phase-dependent flip. | `born_rule_parameter_free.md` (a closed; #2 discriminated); `substrate_determinism.md` (b: non-constructive momentum-dissolution claim) | (b) a construction taking the binary tick to the sine-Gordon shadow (telegrapher's-type, phase-dependent flip supplying the nonlinearity) — closing it also kills the stochastic competitor outright |
