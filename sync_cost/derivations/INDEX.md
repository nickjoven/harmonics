...
| `40_stribeck_vortex.md` | **Derived** | **The Stribeck vortex regime map.** Vortex velocity field v(r) composed with Stribeck friction K(v) yields radial coupling profile K(r) with overcritical core (K > 1, fold, reversed energy flow) and subcritical annulus (K < 1, mode-locked, forward flow). Critical radii are roots of a quadratic; r_c,inner × r_c,outer = r_core² (Vieta, parameter-independent). Inner scales as 1/ℓ, outer as ℓ. Fold measure μ = arccos(1/K)/π. Fold present at all K > 1; chaos generic but not uniform (mode-locking windows persist above K = 1). D36 reconciled: local K > 1 compatible with global K_eff ≤ 1 via triangle inequality on S¹. Photonic crystal: K_stat = 1.636, μ(0) = 29.1% (arXiv:2601.21704). |
