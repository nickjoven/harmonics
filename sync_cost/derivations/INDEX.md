...
| `40_stribeck_vortex.md` | **Derived** | **The Stribeck vortex regime map.** Vortex velocity field v(r) composed with Stribeck friction K(v) yields radial coupling profile K(r) with overcritical core (K > 1, fold, chaos, reversed energy flow) and subcritical exterior (K < 1, mode-locked, forward flow). Critical radius r_c where K(r_c) = 1 scales as 1/ℓ. Fold measure μ = arccos(1/K)/π. D36 reconciled: local K > 1 compatible with global K_eff ≤ 1 via triangle inequality on S¹. Photonic crystal application: K_stat = 1.636, μ(0) = 29.1% reversed flow (arXiv:2601.21704). The K = 1 surface is the event horizon analogue. |
...
| File | Role |
|------|------|
| `40_stribeck_vortex.md` | **Local K > 1 reconciled with global K_eff ≤ 1**: the Stribeck vortex regime map K(r) = K(v(r)) produces radial structure — overcritical core surrounded by subcritical exterior. Critical radius r_c ∝ 1/ℓ. Fold measure μ = arccos(1/K)/π predicts reversed energy flow fraction. Triangle inequality on S¹ preserves global self-consistency. The K = 1 surface at r_c is the event horizon analogue: inside, circle map folds (local information destruction); outside, mode-locking persists. |
| `stribeck_vortex_regime.py` | Full computation: regime map K(r), critical radii vs ℓ, 1/ℓ scaling verification, fold measure profile, Lyapunov exponent sign change at r_c, global order parameter |r| ≤ 1 confirmation, photonic crystal K-mapping (Δn=0.5, f=0.15, Q=50 → K_stat=1.636, μ(0)=29.1%), near-criticality saddle-node scaling. |
