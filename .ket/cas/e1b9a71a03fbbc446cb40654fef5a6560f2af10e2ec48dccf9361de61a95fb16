...
The cube identity `q_3² − q_2³ = 1` (equation L19 above, $\dim
\text{adj}\,\mathrm{SU}(q_3) = q_2^3$) is the specific case
`(p, q) = (2, 3)` of the **Catalan equation**:

$$x^p - y^q = 1$$

Catalan's conjecture (Eugène Charles Catalan, 1844) stated that
the only solution to this equation in positive integers with
`p, q ≥ 2` is `(x, p, y, q) = (3, 2, 2, 3)` — equivalently, 8
and 9 are the only consecutive perfect powers among positive
integers. The conjecture was open for 158 years and was proven
by Preda Mihăilescu (2002; "Primary cyclotomic units and a
proof of Catalan's conjecture", *J. Reine Angew. Math.* **572**
(2004), pp. 167-195) using cyclotomic field methods.

Applied to the framework, Mihailescu's theorem shows that **the
cube identity alone** (without the coupled identity
`q_2² − 1 = q_3`) suffices to force `(q_2, q_3) = (2, 3)`. The
polynomial argument above uses both identities and substitution;
the Mihailescu route uses one identity and a much deeper
number-theoretic result. Under the stronger framing, the
identity `q_2² − 1 = q_3` becomes a *consistency check*
(`2² − 1 = 3` holds for the forced values) rather than a
co-equal forcing input.

**Historical caveat**: the specific case `(p, q) = (2, 3)` of
the Catalan equation — `x² − y³ = 1` — was tractable by
classical methods predating Mihailescu (using the factorization
`x² − 1 = (x − 1)(x + 1)` and requiring the product to be a
perfect cube; this is at the level of mid-19th-century
Diophantine analysis). Mihailescu's contribution was generalizing
the uniqueness result to all `(p, q) ≥ 2`. Citing Mihailescu
provides the strongest available result; the classical argument
suffices for the specific case at hand.

Either framing — the specific case via classical methods, or
the general theorem via Mihailescu — forces the framework's
substrate primitives as the **unique pair of consecutive perfect
powers in the positive integers**. This is the deepest
number-theoretic substrate-forcing currently available for
`(q_2, q_3)`.

By extension, the mass-sector sector constants
`k_lepton = q_3² = 9` and `k_quark = q_2³ = 8` inherit the
Mihailescu-strength forcing through the substrate-derivation
chain — and downstream consequences (the lepton SU(2)
doublet/singlet kinematic split from the walker round-trip
squaring; the quark cube directly from SU(3) adjoint structure)
rest on this number-theoretic foundation.
