...
> **Superseded rows (#263 ⓾ ruling, 2026-07-19).** The registry has
> since adjudicated every row of this table as a bare K=1 identity,
> NOT a prediction at M_Z (`MANIFEST.yml` `bare_k1_identities`;
> `numerology_inventory.md`): `sin²θ_W = 8/35` is **Class 1** with an
> executed running disproof (`sinW_running_check.py` — "do not use as
> building block at M_Z"), and `alpha_s/alpha_2`, `m_H/v`, `lambda`,
> `1/alpha_0` are **Class 3**. Preserved as a historical record.

| Quantity | Computed | Observed | Residual | Registry disposition |
|----------|----------|----------|----------|----------------------|
| alpha_s/alpha_2 | q_3^3/q_2^3 = 27/8 = 3.375 | 3.488 | 3.2% | Class 3, bare K=1 declined |
| sin^2(theta_W) | q_2^3/(q_2^3+q_3^3) = 8/35 | 0.2312 | 1.1% | **Class 1**, running-disproved |
| m_H/v | 1/q_2 = 1/2 | 125.1/246.2 | 1.6% | Class 3, bare K=1 declined |
| lambda (Higgs quartic) | 1/(2q_2^2) = 1/8 | ~0.13 | 4% | Class 3, bare K=1 declined |
| 1/alpha_0 (tree EM) | q_2^3 + q_3^3 = 35 | — | — | Class 3, bare K=1 declined |

The former framing of the 1-4% residuals as "decoherence tax" with
tree-to-M_Z running "matching SM 2-loop running to 0.3% RMS" is the
pre-2026-04 story the registry overturned; see the running disproof
above. (`gate_duty_predictions.py` is retained as the historical
computation.)
...
| alpha_s/alpha_2 | **Superseded — Class 3** (was: Derived, D33) | bare K=1 identity, declined at M_Z; see registry note in Part III |
| sin^2(theta_W) | **Superseded — Class 1** (was: Derived, D33, D37) | running-disproved (`sinW_running_check.py`); see registry note |
| m_H/v | **Superseded — Class 3** (was: Derived, D33) | bare K=1 identity, declined at M_Z |
| lambda | **Superseded — Class 3** (was: Derived, D33) | bare K=1 identity, declined at M_Z |
| 1/alpha_em (tree) | **Superseded — Class 3** (was: Derived, D33) | bare K=1 identity, declined at M_Z |
...
**Derived (partial), rescoped (#263 ⓾ ruling, 2026-07-19).** What
stands: D_0 = 1/2 from the tree geometry, the hierarchy consistency
check (Part II), and theta = 0 (Part V). The five gauge-sector ratio
rows are superseded — bare K=1 identities, not predictions at M_Z; see
the registry dispositions marked in Parts III and IV. The absolute
coupling scales reduce to one dimensionful input (the root oscillator
frequency), which no mathematical framework can produce.
...
grounds: discrete_gauge_resolution.md, gauge_sector_lovelock.md
derives: duty_cycle_dictionary.md, hierarchy_gaussian_lattice.md, planck_scale.md
